-- =====================================================
-- VCM Dashboard - Database Schema for Supabase (SAFE VERSION)
-- =====================================================
...
-- =====================================================
-- Add foreign key constraint for personas -> companies
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_personas_company_id'
    ) THEN
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON system_configurations;
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON personas;

-- Create new policies (allowing all operations - adjust as needed for production)
